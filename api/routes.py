...
async def execute(request: ExecuteRequest, raw_request: RawRequest) -> ExecuteResponse:
    ...
    async def response_listener(
        status: StatusType, prompt: Optional[str] = None
    ) -> None:
...
async def exexcute_input(
    request: ExecuteInputRequest, raw_request: RawRequest
) -> ExecuteResponse:
    ...
    if target_session:
        ...
        async def response_listener(
            ...
            events.remove_listener(
                response_listener, target_session.response_listener_name
            )
            response_event.set()
